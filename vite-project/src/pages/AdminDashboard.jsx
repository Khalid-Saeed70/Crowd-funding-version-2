...
//   const [modalType, setModalType] = useState(null); // users | projects | investments
...
//     const res = await API.get("/admin/investments/all");
//     setInvestments(res.data.data || []);
//     setModalType("investments");
//   };

//   const cardStyle =
//     "bg-black/60 border border-cyan-400/30 backdrop-blur-xl shadow-[0_0_20px_rgba(0,255,255,0.15)] p-5 rounded-2xl hover:scale-105 transition cursor-pointer";
...
//         <div onClick={fetchInvestments} className={`${cardStyle} col-span-2`}>
//           <p className="text-sm text-gray-400">Total Fund</p>
...
//           <div className="relative bg-black/80 border border-cyan-400/30 backdrop-blur-xl text-white p-6 rounded-2xl w-[750px] max-h-[80vh] overflow-auto">

//             {/* ❌ CLOSE BUTTON */}
//             <button
//               onClick={() => setModalType(null)}
//               className="absolute top-3 right-4 text-xl font-bold text-cyan-400"
//             >
//               ✕
//             </button>
...
//                   <p>Deadline: {new Date(p.deadline).toDateString()}</p>
//                 </div>
//               ))}
...
//                   <p><b>Investor:</b> {i.investorId?.name}</p>
//                   <p><b>Email:</b> {i.investorId?.email}</p>
//                   <p><b>Project:</b> {i.projectId?.title}</p>
//                   <p><b>Amount:</b> ₹{i.amount}</p>
//                   <p><b>Method:</b> {i.paymentMethod}</p>
//                   <p><b>Transaction:</b> {i.transactionId}</p>
//                   <p><b>Date:</b> {new Date(i.paymentDate).toDateString()}</p>
    ...
    <AdminLayout>
      ...
      {modalType && (
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50">
          ...
          <div className="relative bg-black/80 border border-cyan-400/30 text-white p-6 rounded-2xl w-[750px] max-h-[80vh] overflow-auto">
            ...
            {modalType === "users" &&
              users.map((u) => (
                <div key={u._id} className="border p-3 mb-3 rounded">
                  ...
                  <div className="flex gap-3 mt-2 flex-wrap text-sm">
                    {u.aadhaarUrl && <a href={u.aadhaarUrl}>Aadhaar</a>}
                    {u.panUrl && <a href={u.panUrl}>PAN</a>}
                    {u.studentIdUrl && <a href={u.studentIdUrl}>Student ID</a>}
                  </div>
                </div>
              ))}
